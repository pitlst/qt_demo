...
void DataGroup::operation_excel_data::read(const std::string & input_name)
{
    // 调用office应用接口
    ...
    QAxObject * sheet = worksheets->querySubObject("Item(QString)", QString::fromStdString(input_name));
    ...
    cell_data[input_name].resize(count);
    for (int var = 0; var < count; ++var)
    {
        QVariantList ch0 = var_list.at(var).toList();
        std::size_t count0 = ch0.size();
        cell_data[input_name].at(var).resize(count0);
        for (int var0 = 0; var0 < count0; ++var0)
        {
            cell_data[input_name].at(var).at(var0) = ch0.at(var0).toString().toStdString();
        }
    }
}
...
void DataGroup::operation_excel_data::write()
{
    ...
    for(const auto & ch : cell_data)
    {
        ...
        int row = ch.second.size();
        int col;
        if(row == 0)
            col = 0;
        else
            col = ch.second[0].size();
        ...
        {
            for (const auto & ch_ : ch.second)
            {
                QList<QVariant> temp_list;
                for (const auto & ch__ : ch_)
                {
                    QVariant temp = QString::fromStdString(ch__);
        ...
        // 写入
        QAxObject * sheet = worksheets->querySubObject("Item(QString)", QString::fromStdString(ch.first));
        if (sheet != nullptr && ! sheet->isNull())
        {
